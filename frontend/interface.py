...
class Vehicle:
    def __init__(self, direction, queue_position, vehicle_type):
        ...
        image_path = os.path.join(img_dir, f"{vehicle_type}.png")

        try:
            self.original_image = pygame.image.load(image_path)
            self.image = pygame.transform.scale(
                self.original_image, (VEHICLE_WIDTH, VEHICLE_HEIGHT))
        except:
    ...
    def get_position(self):
        ...
        if self.direction == "east":
            x = WINDOW_SIZE//2 + ROAD_WIDTH + \
                (self.queue_position * VEHICLE_SPACING)
...
class Intersection:
    ...
    def draw(self):
        ...
        pygame.draw.rect(screen, GRAY, (0, WINDOW_SIZE//2 -
                         ROAD_WIDTH//2, WINDOW_SIZE, ROAD_WIDTH))
        pygame.draw.rect(screen, GRAY, (WINDOW_SIZE//2 -
                         ROAD_WIDTH//2, 0, ROAD_WIDTH, WINDOW_SIZE))
        ...
        if update_function:
            ...
            if new_data:
                ...
                if new_data["vehicles"] != None:
                    ...
                    intersection.define_vehicule_deleted(
                        new_data["vehicle_deleted"])
                else:
