...
def Coordinator(queues, events):
    ...
    threads = []
    threads.append(Thread(target=getQueuesUpdates,
                   args=(queues, events, sock)))

    for direction in CIRCLE_DIRECTION:
        threads.append(Thread(target=manageTrafficForDirection,
                       args=(direction, events, sock)))
